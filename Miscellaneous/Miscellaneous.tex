...
\subsection{账号管理}
\keypoint{登录的用户会自动获得用户ID和组ID。}
\keypoint{passwd每一行由7个字段构成，分别为帐号，密码（x），UID，GID，用户信息，主文件夹，Shell。}
\keypoint{shadow有9个字段，帐号、密码的MD5、最近变更日期、不可变更的天数、需要更改的天数、更改期限前的警告天数、过期后的宽限时间、帐号失效日期、保留字段。}
\keypoint{/etc/group储存用户组，gshadow储存用户组密码。group的字段包含名称、密码、GID、包含的用户等。}
\keypoint{用户可以加入多个用户组，passwd中的用户组名称为初始用户组，用户登陆后拥有其权限。}
\keypoint{usermode -G groupname username可以设置用户组。}
\keypoint{groups命令第一个输出的就是有效用户组，用于创建文件。newgrp可以切换有效用户组。}
\keypoint{useradd可以新建用户，-r创建系统帐号（不会有主文件夹）。}
\keypoint{useradd -D调出/etc/default/useradd获得添加用户的默认操作。注意默认的用户组在centOS下不生效。}
\keypoint{/etc/login.defs包含了各种UIDMIN之类的标准。}
\keypoint{passwd [username]可以设置密码。}
\keypoint{echo SOMETHING | passwd - -stdin username可以从stdin读取设置。}
\keypoint{chage -l 可以详细列出密码参数，不同参数可以修改。}
\keypoint{usermod可以修改账户参数。}
\keypoint{userdel可以删除账户。}
\keypoint{finger命令可以查看用户信息。}
\keypoint{chfn可以修改finger的信息，很鸡肋。}
\keypoint{chsh可以修改用户的shell。}
\keypoint{id可以查看用户的UID，GID等，还可以检测用户是否存在。}
\keypoint{groupadd，groupmod，groupdel与之类似。}
\keypoint{gpasswd设置密码，gpasswd user groupname将用户添加到组，-A将控制权交到他手中。}
\keypoint{ACL可以为每个用户单独分配权限。}
\keypoint{su可以切换身份，默认为root。}
\keypoint{已经跳过第14章后半部分以及第15章。}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{虚拟机}
